...
//      panic( "You need to implement the operations that actually handle the page fault in lab2_3.\n" );
            ...
            {
                pa = (uint64)
                alloc_page(); // allocate a new physical page
                if ((void *) pa == NULL)
                    panic("Can not allocate a new physical page.\n");
                map_pages(current->pagetable, ROUNDDOWN(stval, PGSIZE), PGSIZE, pa, prot_to_type(PROT_READ | PROT_WRITE,
                                                                                                 1)); // maps the new page to the virtual address that causes the page fault
            } else if (*pte & PTE_C) {
